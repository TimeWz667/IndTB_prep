...
load(here::here("pars", "pars_demo_rus.rdata"))
load(here::here("pars", "pars_cascade_rus.rdata"))
...
Targets <- Valid %>% 
  filter(Index %in% c("ARTI", "Prev", "CasePublic", "CasePrivate", "CaseACF", "Inc", "Mor", "PrMDR")) %>% 
...
list(
  Demo = pars_demo_rus,
  Cascade = pars_cascade_rus
) %>% 
jsonlite::write_json(here::here(folder, "pars_rus.json"), auto_unbox = T, digits = 10)
