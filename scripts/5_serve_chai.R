...
load(here::here("pars", "pars_cascade_ru.rdata"))
...
Targets <- Valid %>% 
  filter(Index %in% c("ARTI", "Prev", "CasePublic", "CasePrivate", "CaseACF", "Inc", "Mor")) %>% 
...
list(
  Demo = pars_demo_ru,
  Cascade = pars_cascade_ru
) %>% 
  jsonlite::write_json(here::here(folder, "pars_ru.json"), auto_unbox = T, digits = 10)
